...
2. Verifies it against git (`git fetch`, then four cases reported separately: hash unknown in this clone = repo behind the vault; branch gone = merged or lost, decided against the default branch; hash no longer an ancestor = history rewritten; commits after the hash = work without a checkpoint) and against Linear (`linearis issues read <ID> --fields identifier,title,state.name` vs the recorded status).
