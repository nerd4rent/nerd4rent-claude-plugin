...
Read, never mutate. Use Bash only for read-only commands: `git diff`, `git log`, `git show`, `gh pr diff`, `gh pr view`, `linearis issues read`, `rg`. Do not edit, write, stage, commit, push, or change any tracker or vault state.
